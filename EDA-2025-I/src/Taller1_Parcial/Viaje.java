...
public class Viaje {
    ...
    public LinkedList<Gasto> elegirTipoGasto(String tipo){
        ...
        if(tipo.equals(COMIDA)) listaGastos = comidas;
        if(tipo.equals(HOSPEDAJE)) listaGastos = hospedajes;
        if(tipo.equals(ROPA)) listaGastos = ropa;
        if(tipo.equals(TRANSPORTE)) listaGastos = transportes;
        return listaGastos;
    }
